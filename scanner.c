...
List tokenList(char *ar) {
  ...
  List tl = NULL;
  int i = 0;
  unsigned long length = strlen(ar);
  while (i < length) {
...
/* The function scanEquation is not in the lecture notes.
 * It demonstrates the scanner. Leest input en stopt deze in een linked list.
 * En print de gelezen lijst zolang het geen uitroepteken is.
 */

void scanEquation(void) {
  char *ar;
  List li;
  ar = readInput();
  while (ar[0] != '!') {
